            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('name') is-invalid @enderror" name="primary_email_id">
                        @foreach($user->emails as $email)
                            <option value="{{ $email->id }}" @if(old('primary_email_id', $user->primaryEmail->id) == $email->id) selected @endif>{{ $email->email }}</option>
                        @endforeach
                    </select>
                    <div>
                    <small class="form-hint">The main email address for this user</small>
                    @error('primary_email_id')
                    <p class="invalid-feedback">{{ $message }}</p>
                    @enderror
                    </div>
                </div>
                    ...
                    <div>
                        @foreach($roles as $role)
                            <label class="form-check">
                                <input name="roles[]" class="form-check-input" type="checkbox" value="{{ $role->code }}" @if(in_array($role->code, old('roles', $currentRoles))) checked @endif>
                ...
                <div class="mb-3">
                    <label class="form-check form-switch">
                        <input type="checkbox" class="form-check-input" name="terms" value="1" @if(old('terms', $user->terms_agreed_at)) checked @endif>
                        Accept Terms and Conditions
                    </label>
                    <label class="form-check form-switch">
                        <input type="checkbox" class="form-check-input" name="first_login" value="1" @if(old('first_login', !$user->first_login)) checked @endif>
                        Completed Signup
                    </label>
                    <label class="form-check form-switch">
                        <input type="checkbox" class="form-check-input" name="suspended" value="1" @if(old('suspended', $user->suspended)) checked @endif>
                        Suspended
                    </label>
                </div>
            </div>
